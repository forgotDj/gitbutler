...
fn determine_child_selector<'ws, 'meta, M: RefMetadata>(
    ...
    } else if let Some(child_of_subject) = source_segment.commits.get(index_of_subject_commit - 1) {
...
fn determine_parent_selector<'ws, 'meta, M: RefMetadata>(
    ...
    } else if let Some(parent_of_subject) = source_segment.commits.get(index_of_subject_commit + 1)
    {
